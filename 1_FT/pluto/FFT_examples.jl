...
# ╔═╡ 2f17f11b-a7a4-4ac0-9533-2068a4a417e7
using FFTW

# ╔═╡ bf9787d7-8343-4989-96ff-bef9880aca32
using Plots

# ╔═╡ 606ac79c-95dc-4ed4-b71a-3fe0d86018ce
using DSP

# ╔═╡ a3c7724e-92c8-43d9-803b-ed87ebe6f22b
using PGFPlotsX

# ╔═╡ e14c2279-a350-432b-9ef7-46e64e2d65f7
begin
		n_sample = 12
		ts = 1/8
...
let
	...
	@pgf    p = PGFPlotsX.Plot(
	        { mark = "o", thick},
	      		Table([0: n_sample-1, f_cos ])   
			)
	push!(myaxis, p)

	@pgf    p = PGFPlotsX.Plot(
	        { no_marks, thick},
	      		Table([0: 2*n_sample-1, [f_cos; f_cos] ])   
			)
	push!(myaxis, p)

	@pgf    p = PGFPlotsX.Plot(
	        { no_marks},
	      		Table([0: 2*n_sample-1, f_cos2 ])   
			)
	push!(myaxis, p)

	
	push!(myaxis, raw"\draw[dashed] (11.5,-2) -- (11.5,2);")
				
	pgfsave("../clipping_artefact.tikz.tex",myaxis; include_preamble= false)
	myaxis
end

# ╔═╡ 2dc7e05a-4d98-4e24-acd4-7203c59f0cb6
begin
	#n_sample2 = 12
	...
	f1 =fftshift(fftfreq(length(F_cos), 1/ts))
	FT1 = fftshift(real.(F_cos))./n_sample

	f2 = fftshift(fftfreq(length(F_padded), 1/ts))
	FT2 = fftshift(real.(F_padded))./n_sample
	
	scatter(f1, FT1,
		yrange=(-0.1,0.6), xlabel="Frequenz", ylabel="F_cos", label="orginal")
	
	plot!(f2, FT2, label="padded")
	
	scatter!([-1, 1], [0.5, 0.5], label="ideal")
end

# ╔═╡ dda50ab2-7950-4469-a3b4-7e298e2a1f89
let
...
@pgf    p = PGFPlotsX.Plot(
	...
	pgfsave("../zeropadding.tikz.tex",myaxis; include_preamble= false)
	myaxis
end

# ╔═╡ e891d79b-a1bb-4838-80be-9b3df4a429a7
begin
	#n_sample3 = 12
	...
	#f1 =fftshift(fftfreq(length(F_cos), 1/ts))
	FT1a = fftshift(abs.(F_cos))./n_sample

	#f2 = fftshift(fftfreq(length(F_padded), 1/ts))
	FT2a = fftshift(abs.(F_padded))./n_sample
	...
	FT3a = fftshift(abs.(F_windowed))./n_sample

		scatter(f1, FT1a,
		yrange=(-0.1,0.6), xlabel="Frequenz", ylabel="F_cos", label="orginal")
	
	plot!(f2, FT2a, label="padded")
	plot!(f2, FT3a, label="windowed",linewidth = 4 )
...
# ╔═╡ d38c0a12-7ee6-4408-8b1e-d011f9913b15
let
...
@pgf    p = PGFPlotsX.Plot(
	...
	pgfsave("../zeropadding_window.tikz.tex",myaxis; include_preamble= false)
	myaxis
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "aaa50a1a82ecbbb2ed76ab5d903ac2580acf8077"
