...
# ╔═╡ 511574be-db07-4e7b-a67a-580f843e7135
using Plots, DSP,PGFPlotsX,FFTW

# ╔═╡ 0ef65869-7149-47d2-b510-689adac8cb8c
begin
...
# ╔═╡ 4c88d98c-5177-4395-ab02-b3ee26db620e
winlst = ["rect", "hanning", "hamming", "cosine", "lanczos", "triang", "gaussian","kaiser"]

# ╔═╡ 88f3a842-4df0-438c-8ed8-dd83bfe5e72f
function myfft(wn)
	window2 = winlst[wn]
	
	if (window2=="gaussian")
		expr = "DSP.Windows.$(window2)($(length(signal)),0.1)"
	else
		if (window2=="kaiser") 
			expr = "DSP.Windows.$(window2)($(length(signal)),2)"
		else
	...
	x = fftshift(fftfreq(length(F_windowed), 1/dt))
	y = fftshift(F_windowed)./maximum(F_windowed)
	
	return [x,y]
end

# ╔═╡ e9a4fb9d-a322-4f23-a171-d49863097b21
window2 = winlst[1]

# ╔═╡ cc5c5eb7-a673-4050-af9c-54456a3d72c4
let
	fl = [1 1.15   1.25   2     2.27      3]
	Al = [1 1e-2   1e-3  1e-3    1e-4   1e-5]
...
# ╔═╡ 0366b4e5-6d73-4d56-bc52-013a4931ab1b
fl

# ╔═╡ e6213d94-d988-46c9-8c59-094fbb604fa5
[fl, Al]

# ╔═╡ e2487c8a-4e2b-11ee-1bed-991b1c47b04d
let
	...
	@pgf    p = PGFPlotsX.Plot(
	        { "only marks", mark = "*", thin},
	      		Table([[1, 1.15 ,  1.25  , 2  ,   2.27  ,    3],
					[1, 1e-2  , 1e-3 , 1e-3  ,  1e-4   , 1e-5]])   
			)
	push!(myaxis, p)
	...
	pgfsave("../example_rect.tikz.tex",myaxis; include_preamble= false)
	myaxis
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "36548bd2287ed941262e9c98968c528d2c03459e"
